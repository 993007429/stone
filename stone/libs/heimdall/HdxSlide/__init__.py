...
class HdxSlide(SlideBase):
    ...
    def read(self, location=[0, 0], size=None, scale=1, greyscale=False):
        ...
        if crop_level > 2:
            ...
            xn = int(x + width)
            finalpic = os.path.splitext(self.filename)[
                           0] + os.sep + 'Thumbs' + os.sep + 'Result-%s.jpg' % self.config.get('General', 'SlideID')
            ...
            for k in range(len(self.picinfo)):  # 循环遍历所有小的切片
                if self.picinfo[k]['x'] + self.tilewidth < x or self.picinfo[k]['x'] > xn or self.picinfo[k][
                    'y'] + self.tileheight < y or self.picinfo[k]['y'] > yn:  # 和要找的tile不相交就跳过
                    continue
                try:
                    jpg = Image.open(os.path.splitext(self.filename)[0] + os.sep + 'Images\\IMG'.replace('\\', os.sep) +
                                     self.picinfo[k]['filename'] + '.jpg')  # 打开和要找的tile相交的小图
                    jpg = jpg.resize((int(self.tilewidth / scale), int(self.tileheight / scale)))
                    crop_region.paste(jpg, (int(int(self.picinfo[k]['x'] - x) / scale),
                                            int(int(self.picinfo[k]['y'] - y) / scale)))  # 将需要的信息画在crop_region上
                except:
                    pass
    ...
    def get_thumbnail(self, size):
        ...
        finalpic = os.path.splitext(self.filename)[0] + os.sep + 'Thumbs' + os.sep + 'Result-%s.jpg' % self.config.get(
            'General', 'SlideID')
        img = cv2.imdecode(np.fromfile(finalpic, dtype=np.uint8), cv2.IMREAD_COLOR)
    ...
    def convert_pos(self, x, y):  # 这个边界判断还需要调试的时候加上
        for k in range(len(self.picinfo)):
            if self.picinfo[k]['x'] + self.tilewidth > x and self.picinfo[k]['y'] + self.tileheight > y:
                # print(k)
                # print('x',self.picinfo[k]['x'],'y',self.picinfo[k]['y'])
                # print('前端传参',x,y)
                # print(self.picinfo[k]['filename'])
                ...
                offsetz = 5
                # print('计算后',xPos+offsetx,yPos+offsety,zPos+offsetz)
                # return  xPos-offsetx+self.backgroundImagePosY,yPos+offsety+self.backgroundImagePosY,zPos-offsetz
