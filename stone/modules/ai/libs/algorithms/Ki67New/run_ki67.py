...
from stone.modules.ai.libs.algorithms.Ki67New.src.utils import delete_prev_json, split_patches, split2groups, dump_results, map_results, roi_filter, split_patches_map
from stone.modules.ai.libs.algorithms.Ki67New.src.seg_tissue_area import find_tissue_countours
from stone.modules.ai.libs.algorithms.Ki67New.src.cell_det_cls import cal_pdl1_np
from stone.modules.ai.libs.algorithms.Ki67New.models.detr import build_model
...
def model_selection():
    ...
    if model_type == 'general':
        ...
        net_weights_model = oss.get_object_to_io(oss.path_join('AI', 'Ki67New', 'Model', model_path))
        net_weights_dict = torch.load(net_weights_model, map_location=lambda storage, loc: storage)
        mean_std_data = oss.get_object_to_io(
            oss.path_join('AI', 'Ki67New', 'Model', 'mean_std', config.get(model_type, 'meanstdVersion')))
...
def load_model(default_device):
    ...
    net.load_state_dict(net_weights_dict['model'] if 'model' in net_weights_dict else net_weights_dict)
    net.eval()
...
def compute_process(slide_path, x_coords=[], y_coords=[], patch_size=1024):
    ...
    if comm_rank == 0:
        ...
        if len(x_coords) > 0:
            ...
            if contours == []:
                ...
                region_info_dict = split_patches_map(slide, patch_size=int(patch_size/resize_ratio), threshold_map=threshold_map)
    ...
    # print('local num: {}'.format(len(local_data_dict)))
    ...
    for key, value in local_data_dict.items():
        try:
            region_start = value
            cur_region = slide.read(([region_start[0], region_start[1]]), (int(patch_size/resize_ratio), int(patch_size/resize_ratio)), 1)
            cur_shape = cur_region.shape
            cur_region = cv2.resize(cur_region,(int(cur_shape[1]*resize_ratio),int(cur_shape[0]*resize_ratio)))
            test_center_coords, test_labels, test_probs = cal_pdl1_np(cur_region.astype(np.uint8), mean, std, net, int_device)
