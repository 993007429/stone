...
def imshow_det_bboxes(img,
    ...
    for i, (signal_points, signal_labels) in enumerate(zip(gt_signal_points, gt_signal_labels)):
        if len(signal_labels) > 0:
            assert len(signal_points) == len(signal_labels)
            thickness = -1
            radius = 1
            signal_color_dict = {1:(255, 0, 0), 2:(0, 255, 0)}
            for point, signal_label in zip(signal_points.numpy(), signal_labels.numpy()):
                img = cv2.circle(img, (int(point[0]), int(point[1])), radius,
                                 signal_color_dict[signal_label], thickness)
