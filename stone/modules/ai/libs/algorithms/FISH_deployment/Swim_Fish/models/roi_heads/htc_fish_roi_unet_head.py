...
from stone.modules.ai.libs.algorithms.FISH_deployment.Swim_Fish.core.post_processing.signal_aug import get_aug_signals
...
class HTCFishRoIUNetHead(CascadeSignalRoIHead):
    ...
    def _mask_forward_train(self,
        ...
        mask_results = dict(mask_pred=mask_pred, mask_feats=mask_feats, loss_mask=loss_mask)

        return mask_results

    def _signal_forward_train(self, x, sampling_results, mask_last_feat, gt_signal_points, gt_signal_labels, gt_signal_ignore_area):
        """Run forward function and calculate loss for mask siganl in training."""
        rois = bbox2roi([res.pos_bboxes for res in sampling_results])
        num_rois = len(rois)
        # print('************* rois see signal *************', len(rois), rois[0].size(), rois[0])
        x = [x[i] for i in range(len(x)-1, -1, -1)][2:-1]
        last_level_feature = mask_last_feat
        for each_level in range(len(x)):
            cur_level_roi_extractor = self.signal_roi_extractor[each_level]
            cur_level_signal_head = self.signal_head[each_level]
            cur_level_feats = x[each_level].unsqueeze(0)
            cur_level_roi_feats = cur_level_roi_extractor(cur_level_feats, rois)
            if each_level == 0:
                cur_features_sum = cur_level_roi_feats
            else:
                upsample_feats = F.interpolate(last_level_feature, scale_factor=2, mode='bilinear', align_corners=False)
                cur_features_sum = torch.cat([cur_level_roi_feats, upsample_feats], dim=1)
            last_level_feature = cur_level_signal_head(cur_features_sum)

        deltas, pred_logits = cur_level_signal_head.forward_signal(last_level_feature)
        mask_feature_size = cur_level_roi_extractor.roi_layers[0].output_size[0]
        print('mask_feature_size', mask_feature_size)
        rel_roi_anchor_points = self.set_anchor(num_rois, mask_feature_size = mask_feature_size)
        rel_roi_anchor_points = rel_roi_anchor_points.cuda(deltas.device)
        points_in_roi_small_image = deltas * 0.1 + rel_roi_anchor_points
        pred_coords = self.get_points_in_roi_small_image(rois, points_in_roi_small_image)
        target_signals = cur_level_signal_head.get_targets(sampling_results, gt_signal_points, gt_signal_labels, gt_signal_ignore_area)
    ...
    def _signal_forward_test(self, x, mask_rois, mask_last_feat):
        rois = mask_rois
        num_rois = len(rois)
        x = [x[i] for i in range(len(x) - 1, -1, -1)][2:-1]
        last_level_feature = mask_last_feat
        ...
        return pred_abs_points, pred_logits
    ...
    def get_tareget_abs_points(self, rois, target_points):
        target_abs_points = []
    ...
    def forward_train(self,
        ...
        loss_signal = self._signal_forward_train(x, sampling_results, mask_results['mask_feats'],
                                                 gt_signal_points, gt_signal_labels, gt_signal_ignore_area)
        losses['loss_signal'] = loss_signal
