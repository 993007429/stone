...
from .base_fish_roi_head import BaseSignalRoIHead


@HEADS.register_module()
class CascadeSignalRoIHead(BaseSignalRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def __init__(self,
        ...
        super(CascadeSignalRoIHead, self).__init__(
            bbox_roi_extractor=bbox_roi_extractor,
            bbox_head=bbox_head,
            mask_roi_extractor=mask_roi_extractor,
            mask_head=mask_head,
            signal_roi_extractor=signal_roi_extractor,
    ...
    def init_signal_head(self, signal_roi_extractor, signal_head):
        """Initialize mask head and mask roi extractor.
        ...
        # print('################# HERE ##########################')
        print(signal_head)
        self.signal_roi_extractor = ModuleList()
        self.signal_head = nn.ModuleList()
        if not isinstance(signal_roi_extractor, list):
            signal_roi_extractor = [
                signal_roi_extractor for _ in range(self.num_levels)
            ]
        if not isinstance(signal_head, list):
            signal_head = [signal_head for _ in range(self.num_levels)]
        print('########## ',  len(signal_head), len(signal_roi_extractor), self.num_levels)
        assert len(signal_head) == len(signal_roi_extractor) == self.num_levels
        # signal_head = list(signal_head)
        for roi_extractor, head in zip(signal_roi_extractor, signal_head):
            self.signal_roi_extractor.append(build_roi_extractor(roi_extractor))
            self.signal_head.append(build_head(head))
